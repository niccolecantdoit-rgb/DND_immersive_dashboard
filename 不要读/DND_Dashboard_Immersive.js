    ...
    (function () {
        ...
        // ==========================================
        // 2.5 IndexedDB 适配器 (Avatar Storage)
        // ==========================================
        const DBAdapter = {
        ...
        // Helper for safe storage (Modified to use IndexedDB + LocalStorage Backup with Quota handling)
        ...
        const TavernAPI = {
            ...
            getPresets: function() {
                ...
                // 调试: 打印 extensionSettings 结构以排查路径问题
                // console.log('[TavernAPI] Settings:', SillyTavern.extensionSettings);
        ...
        const PresetSwitcher = {
            ...
            checkCombatStateChange: function(isCombat) {
                ...
                if (isCombat && (!this._lastCombatState)) {
                    if (UIRenderer.initResourceTracker) UIRenderer.initResourceTracker();
                }
...
不要使用markdown代码块。直接返回 <svg ...> ... </svg>。`
        ...
        // 核心辅助函数：获取正确的上下文（兼容 iframe 和父级窗口）
        // [修复] UI 操作使用当前窗口的 jQuery，API 从父窗口获取
        const getCore = () => {
            try {
                const w = window.parent || window;
                // [关键修复] jQuery 优先使用当前窗口的，确保 DOM 操作正确
                const localJQuery = window.jQuery;
                const parentJQuery = w.jQuery;
                // 如果当前窗口有 jQuery 则用当前的，否则用父窗口的
        ...
        const UpdateController = {
            ...
            handleUpdate: () => {
                ...
                if ($('#dnd-dashboard-root').hasClass('visible')) {
                    ...
                    if ($activeItem.length) UIRenderer.renderPanel($activeItem.data('target'));
                }
                // 始终渲染/更新简略 HUD
        ...
        const addStyles = () => {
            ...
            const css = `
                :root {
                    ...
                    --dnd-border-gold: #9d8b6c;
                    --dnd-border-inner: #4a453a;
                    --dnd-text-main: #cbbba8;
                ...
                /* 卡片通用样式 */
                .dnd-char-card {
                    background: linear-gradient(135deg, rgba(30,30,32,0.95) 0%, rgba(20,20,22,0.98) 100%) !important;
                ...
                .dnd-hud-header {
                    ...
                    background: linear-gradient(to right, #202022, #161618) !important; /* 稍微提亮背景 */
                    border-bottom: 1px solid var(--dnd-border-gold) !important; /* 改用金色边框 */
                    font-size: 14px !important;
                    color: var(--dnd-text-main) !important;
                ...
                .dnd-hud-quests {
                    background: rgba(0,0,0,0.3) !important; padding: 8px !important; border-radius: 4px !important;
                }
                .dnd-hud-quest-item {
                    display: flex !important; justify-content: space-between !important; font-size: 13px !important;
                    padding: 4px 0 !important; border-bottom: 1px dashed rgba(255,255,255,0.1) !important;
                    cursor: pointer !important;
                }
                .dnd-hud-quest-item:hover { color: var(--dnd-text-highlight) !important; }
                ...
                .dnd-mini-char {
                    ...
                    background: rgba(255,255,255,0.05) !important; padding: 5px 10px !important; border-radius: 4px !important;
                    border-left: 3px solid transparent !important;
                    cursor: pointer !important;
                    transition: background 0.2s !important;
                }
                .dnd-mini-char:hover { background: rgba(255,255,255,0.1) !important; }
                .dnd-mini-char.active { border-left-color: var(--dnd-text-highlight) !important; background: rgba(255,219,133,0.05) !important; }
                
                .dnd-mini-char-avatar {
                    width: 32px !important; height: 32px !important; border-radius: 50% !important; background: #333 !important;
                    border: 1px solid var(--dnd-border-gold) !important; display: flex !important; align-items: center !important; justify-content: center !important;
        ...
        const DataManager = {
            ...
            // [新增] 查找表键名 (解决 hardcoded key 问题)
            findTableKey: (data, fragment) => {
                if (!data) return null;
                return Object.keys(data).find(k =>
                    k === fragment ||
                    k.includes(fragment) ||
                    (data[k].name && data[k].name.includes(fragment))
                );
            },
            ...
            getCharacterSkills: (charId) => {
                ...
                const charLinks = links.filter(l => l['CHAR_ID'] === charId);
        ...
        const UIRenderer = {
            ...
            setControlledCharacter: (charId) => {
                UIRenderer._controlledCharId = charId;
            ...
            getControlledCharacter: () => {
                if (UIRenderer._controlledCharId) {
                    const party = DataManager.getPartyData();
                    return party.find(p =>
                        p['CHAR_ID'] === UIRenderer._controlledCharId ||
                        p['PC_ID'] === UIRenderer._controlledCharId ||
                        p['姓名'] === UIRenderer._controlledCharId
                    );
                }
                return UIRenderer.getCurrentActiveCharacter();
            },
            ...
            adjustTurnResource: (type) => {
                ...
                if (type === 'movement') {
                    // 移动力增加 30尺
                    UIRenderer._turnResources.movement += 30;
                } else {
                    // 其他资源 +1
                    if (UIRenderer._turnResources[type] !== undefined) {
                        UIRenderer._turnResources[type]++;
                    }
                }
                UIRenderer.renderHUD();
            ...
            resetActionEconomy: () => {
                const char = UIRenderer.getControlledCharacter();
                // 尝试从属性读取速度
                let speed = 30;
                if (char && char['速度']) {
            ...
            initResourceTracker: () => {
                ...
                UIRenderer._resourceTracker.snapshots[charId] = {
            ...
            calculateResourceConsumption: () => {
                ...
                const charId = char['CHAR_ID'] || char['PC_ID'] || char['姓名'];
                const snapshot = UIRenderer._resourceTracker.snapshots[charId];
            ...
            renderResourceConsumption: ($container) => {
                const consumption = UIRenderer.calculateResourceConsumption();
                if (!consumption) {
                    $container.html('');
                    return;
                }
            ...
            // [新增] 切换仪表盘状态的辅助方法
            toggleDashboard: () => {
                Logger.info('toggleDashboard 被调用，当前状态:', UIRenderer.state);
                UIRenderer._lastToggleTime = Date.now();
                
                if (UIRenderer.state === 'collapsed') {
                    UIRenderer.setState('mini');
                } else {
                    UIRenderer.setState('collapsed');
                }
            },
            ...
            handleCastClick: (name, range, level, type, costType) => {
                ...
                UIRenderer.hideDetailPopup();
                
                if (isCombat) {
                    // 战斗状态：进入瞄准模式
                    UIRenderer.startTargeting({
                        type: type || 'skill',
                        rangeText: range,
                        skillName: name,
                        costType: costType || 'Action', // 默认标准动作
                        callback: (res) => UIRenderer.executeAction(type || 'skill', res, costType || 'Action')
                    });
                } else {
                    // 非战斗状态：直接输出文本
                    let text = '';
            ...
            prepareCast: (spellName, rangeText, baseLevelStr) => {
                ...
                // 如果是戏法，直接调用处理
                if (baseLevel === 0) {
                    UIRenderer.handleCastClick(spellName, rangeText, '0', 'spell');
                    return;
                }
                
                // 获取当前角色及其法术位
                const pc = UIRenderer.getControlledCharacter();
                ...
                if ($popup.length) {
                    ...
                    // 替换当前弹窗内容
                    // 注意：这里我们保留关闭按钮，只替换内容容器
                    // 但是 showItemDetailPopup 的结构是 CloseBtn + Div
                    // 所以我们直接替换 popup 内部的 div
                    const $content = $popup.find('> div:not(:first-child)'); // 假设第一个是 close btn (wait, check showItemDetailPopup impl)
                    // showItemDetailPopup: $popup.html(closeBtn + '<div style="padding-right:20px;">' + contentHtml + '</div>');
                    // closeBtn is a div.
                    if ($content.length) {
                        $content.html(html);
                    } else {
            ...
            startTargeting: (config) => {
                ...
                const range = UIRenderer.parseDistance(rangeText);
                
                // 检查资源是否足够 (提前检查)
                if (type !== 'move' && costType) {
                    const key = costType.toLowerCase();
                    if (UIRenderer._turnResources[key] <= 0) {
                        alert(`❌ 无法执行：${costType} 资源已耗尽！`);
                        return;
                    }
                }
                
                UIRenderer._targetingMode = {
                    active: true,
                    type: type || 'skill',
                ...
                UIRenderer.showItemDetailPopup(`<div style="text-align:center;color:var(--dnd-accent-green);font-weight:bold;">🎯 ${msg}</div>`, coreWin.innerWidth/2, 100);
            },

            // [新增] 结束瞄准模式
            endTargeting: () => {
                UIRenderer._targetingMode = { active: false, type: null, source: null, range: 0 };
                // 清除虚拟位置，以确保下次瞄准从真实位置开始 (或者在 executeAction 中更新)
                // 如果 executeAction 更新了 virtualPos, 这里不需要清除，除非取消操作
                // 但如果成功执行了动作，targetingMode.callback 会调用 executeAction，然后 endTargeting
                // 所以这里保留 virtualPos 不清除，以便后续行动连续
            ...
            getGridFromEvent: (e, $container, $innerMap) => {
                ...
                // 获取当前的变换状态
                const state = UIRenderer._mapZoom;
                ...
                // 反向计算相对于 innerMap 中心的偏移
                // 注意：state.panX 和 state.panY 是 translateY/X 的值，已经受 scale 影响?
                // 不，transform 是 scale() translate() 顺序，通常 scale 会影响 translate 的视觉效果
                // 但我们的 applyTransform 是 `scale(${state.scale}) translate(${state.panX}px, ${state.panY}px)`
                // 这意味着 translate 是在 scale 之后应用的坐标系? 不，CSS transform 函数顺序很重要。
                // scale(s) translate(tx, ty) 等价于先缩放，再平移。平移量是在缩放后的坐标系中? 不，标准CSS中:
                // transform: scale(2) translate(10px, 0) -> 放大2倍，然后向右平移 10*2 = 20px (如果translate在scale后? Wait)
                // 如果是 scale(s) translate(x, y)，实际上 translate 是在 s 倍的坐标系上移动 x, y
                
                // 让我们修正反算逻辑。
                // 屏幕点击点 (clickX, clickY) 相对于容器中心 (cx, cy)
                // 变换中心是 'center center' (即 cx, cy)
                // 变换公式: P_screen = (P_local - Center) * scale + Center + Pan * scale (如果是这种顺序?)
                
                // 我们的 CSS: transform: scale(s) translate(px, py); transform-origin: center center;
                // 这意味着:
                // 1. 以 center 为原点，缩放 s 倍
                // 2. 然后平移 (px, py) * s ? 或者只是 px, py?
                // 实测表明：scale(s) translate(x,y) -> translate 的单位也会被 scale 影响。
            ...
            // 异步加载头像
            loadAvatarAsync: async (charId, elemId) => {
                const { $ } = getCore();
                const base64 = await UIRenderer.avatarStorage.get(charId);
                if (base64) {
                    const $el = $(`#${elemId}`);
                    if ($el.length) {
                        ...
                        $el.find('img').on('error', function() {
                            $(this).hide();
                            // 重新插入字母
                            const initial = UIRenderer.getNameInitial($el.attr('title'));
            ...
            renderAvatar: (name, charId, size = 40) => {
                const initial = UIRenderer.getNameInitial(name);
                const fontSize = Math.floor(size * 0.5);
                // 生成唯一ID以便异步填充
                const uid = `avatar-${charId}-${Math.random().toString(36).substr(2, 9)}`;
                
                // 触发异步加载
                setTimeout(() => UIRenderer.loadAvatarAsync(charId, uid), 0);
            ...
            showAvatarUploadDialog: (charId, charName) => {
                ...
                const initial = UIRenderer.getNameInitial(charName);
                ...
                $('#dnd-avatar-file-input').on('change', function(e) {
                    ...
                    reader.onload = function(evt) {
                        ...
                        // 压缩图片
                        UIRenderer.compressImage(base64, 150, (compressedBase64) => {
                            // 保存到 localStorage
                            if (UIRenderer.avatarStorage.set(charId, compressedBase64)) {
                                // 更新预览
                                $('#dnd-avatar-preview').html(`<img src="${compressedBase64}" style="width:100%;height:100%;object-fit:cover;">`);
                                
                                // 更新页面上所有该角色的头像
                                UIRenderer.refreshAvatars(charId);
                ...
                $('#dnd-avatar-remove-btn').on('click', () => {
                    if (confirm('确定要移除头像吗？')) {
                        UIRenderer.avatarStorage.remove(charId);
                        UIRenderer.refreshAvatars(charId);
                        $('#dnd-avatar-upload-dialog, #dnd-avatar-dialog-backdrop').remove();
                    }
                });
            ...
            init: () => {
                const { $ } = getCore();
                if ($('#dnd-dashboard-root').length) return;
                        ...
                        try {
                            ...
                            if (pos && pos.left) {
                                ...
                                if (btn) {
                                    ...
                                    // 初始位置恢复后更新 HUD
                                    setTimeout(() => UIRenderer.updateHUDPosition(), 100);
                                }
                ...
                // 监听窗口调整
                $(window).on('resize.dnd', () => UIRenderer.updateHUDPosition());

                // [新增] 定时器强制更新位置 (防止样式被覆盖或初始化失败)
                setInterval(() => UIRenderer.updateHUDPosition(), 1000);
                ...
                setTimeout(() => {
                    ...
                    Logger.debug('  - 该位置顶���元素:', elementAtPoint);
                ...
                $('#dnd-close').on('click', () => UIRenderer.setState('mini'));
                
                // Logo 点击事件：切换完整面板
                $('#dnd-logo-container').on('click', function(e) {
                    e.stopPropagation();
                    // 简单的动画反馈
                    $(this).css('transform', 'scale(0.9)');
                    setTimeout(() => $(this).css('transform', ''), 150);
                    
                    UIRenderer.setState('full');
                });
                ...
                // [新增] 全局快捷键支持
                $(document).on('keydown.dndHotkeys', function(e) {
                    // 忽略输入框内的按键
                    ...
                    if (UIRenderer.state === 'mini' && key >= '1' && key <= '9') {
                        const idx = parseInt(key) - 1;
                        const party = DataManager.getPartyData();
                        if (party && party[idx]) {
                            const { window: coreWin } = getCore();
                            UIRenderer.showCharacterCard(party[idx], { clientX: coreWin.innerWidth / 2, clientY: 100 });
                            e.preventDefault();
                            return;
                        }
                    }
                ...
                // 全局事件委托：头像双击上传
                $(document).on('dblclick', '.dnd-avatar-container', function(e) {
                    e.stopPropagation();
                    const charId = $(this).data('char-id');
                    const charName = $(this).attr('title') || $(this).find('span').text() || $(this).find('.dnd-avatar-initial').text() || '角色';
                    ...
                    if (charId) {
                        ...
                        if (isPartyMember) {
                            UIRenderer.showAvatarUploadDialog(charId, charName);
                        } else {
            ...
            setState: (newState) => {
                ...
                switch (newState) {
                    ...
                    case 'full':
                        ...
                        if ($active.length) {
                            UIRenderer.renderPanel($active.data('target'));
                        } else {
                            UIRenderer.renderPanel('party');
                        }
                        break;
                }
            },
            ...
            renderPanel: (panelName) => {
                ...
                switch (panelName) {
                    case 'create':
                        UIRenderer.renderCharacterCreationPanel($content);
                        break;
                    case 'party':
                        UIRenderer.renderPartyPanel($content);
                        break;
                    case 'quests':
                        UIRenderer.renderQuestsPanel($content);
                        break;
                    case 'inventory':
                        UIRenderer.renderInventoryPanel($content);
                        break;
                    case 'combat':
                        UIRenderer.renderCombatPanel($content);
                        break;
                    case 'world':
                        UIRenderer.renderWorldPanel($content);
                        break;
                    case 'logs':
                        UIRenderer.renderLogsPanel($content);
                        break;
                    case 'npcs':
                        UIRenderer.renderNPCPanel($content);
                        break;
                    case 'archives':
                        UIRenderer.renderArchivesPanel($content);
                        break;
                    case 'settings':
                        UIRenderer.renderSettingsPanel($content);
                        break;
                    default:
            ...
            renderSettingsPanel: async ($c) => {
                ...
                $c.find('#dnd-set-fetch-models').on('click', async function(e) {
                    ...
                    if (!url) { alert('请输入 API URL'); return; }
                    ...
                    try {
                        ...
                        alert(`获取成功，共 ${models.length} 个模型。请从下拉列表��选择。`);
                    } catch(e) {
            ...
            renderPartyPanel: ($container) => {
                ...
                party.forEach((char, index) => {
                    ...
                    const avatarHtml = UIRenderer.renderAvatar(char['姓名'], charId, 40);
                    ...
                    $card.on('click', (e) => {
                        ...
                        // 传递点击事件以便在鼠标位置显示卡片
                        UIRenderer.showCharacterCard(char, e);
                    });
            ...
            renderInventoryPanel: ($c) => {
                ...
                if (equippedItems.length > 0) {
                    ...
                    equippedItems.forEach((i, idx) => {
                        const card = UIRenderer.renderItemCard(i, true, idx * 0.05);
                        // 为卡片添加 data 属性以便筛选
                        const $card = $(card);
                ...
                Object.keys(categories).forEach(cat => {
                    ...
                    catItems.forEach((i, idx) => {
                        const card = UIRenderer.renderItemCard(i, false, idx * 0.05);
                        const $card = $(card);
                        $card.attr('data-name', i['物品名称']);
            ...
            showItemDetail: (itemId, event) => {
                ...
                const { window: coreWin } = getCore();
                UIRenderer.showItemDetailPopup(html, event ? event.clientX : coreWin.innerWidth/2, event ? event.clientY : coreWin.innerHeight/2);
            },
            ...
            renderNPCPanel: ($c) => {
                ...
                npcs.forEach((npc, index) => {
                    ...
                    $card.on('click', () => {
                        ...
                        UIRenderer.showModal(npc['姓名'], detail);
                    });
            ...
            // [新增] 保存角色创建状态
            saveCreatorState: () => {
                if (UIRenderer._charCreatorState) {
                    safeSave('dnd_creator_state', JSON.stringify(UIRenderer._charCreatorState));
                }
            },
            ...
            renderCharacterCreationPanel: ($container) => {
                ...
                if (!UIRenderer._charCreatorState) {
                    ...
                    DBAdapter.getSetting('dnd_creator_state').then(savedState => {
                        UIRenderer._charCreatorLoading = false;
                        
                        if (savedState) {
                            try {
                                UIRenderer._charCreatorState = typeof savedState === 'string' ? JSON.parse(savedState) : savedState;
                        ...
                        if (!UIRenderer._charCreatorState) {
                            ...
                            // [修复] 改为使用 SettingsManager.getAPIConfig() 统一获取，支持多源回退
                            SettingsManager.getAPIConfig().then(config => {
                                if (config && config.url) {
                                    if (UIRenderer._charCreatorState) {
                                        UIRenderer._charCreatorState.apiConfig = config;
                                    }
                                }
                                // 加载配置后刷新显示
                                UIRenderer.renderCharacterCreationPanel($container);
                            });
                            return; // 等待回调刷新
                        }
                ...
                const state = UIRenderer._charCreatorState;
                    ...
                    state.conversationHistory.forEach((msg, idx) => {
                        ...
                        chatHistoryHtml += `
                            <div class="dnd-chat-msg dnd-anim-entry" style="animation-delay:${idx * 0.05}s; background:${bgColor}; border-left:3px solid ${borderColor}; padding:10px 12px; margin-bottom:8px; border-radius:4px;">
                                ...
                                <div style="color:var(--dnd-text-main);line-height:1.5;white-space:pre-wrap;">${UIRenderer.formatChatMessage(msg.content)}</div>
                            </div>
                        `;
                    });
                }
                            ...
                            <div style="flex:1;min-width:250px;">
                                <div style="background:rgba(0,0,0,0.2);border:1px solid var(--dnd-border-gold);border-radius:6px;overflow:hidden;">
                                    ...
                                    <div id="dnd-creator-preview" style="padding:15px;min-height:300px;">
                                        ${UIRenderer.renderCharacterPreview(state.characterData)}
                                    </div>
                                </div>
                ...
                // 绑定事件
                UIRenderer.bindCharacterCreatorEvents($container);
            },
            ...
            showStatsGenerator: (e) => {
                ...
                UIRenderer.showItemDetailPopup(html, e.clientX, e.clientY);
            },
            ...
            applyStatsOption: (type) => {
                ...
                if (text) {
                    UIRenderer.fillCreatorInput(text);
                    UIRenderer.hideDetailPopup();
                }
            },
            ...
            confirmStatsRoll: () => {
                ...
                if (res) {
                    UIRenderer.fillCreatorInput(`我选择使用【骰子投掷结果】：[${res}]。请帮我分配到合适的属性上。`);
                    UIRenderer.hideDetailPopup();
                }
            },
            ...
            bindCharacterCreatorEvents: ($container) => {
                ...
                // (API 配置已移至 Settings，此处仅做状态展示，无需绑定输入事件)
                
                // 角色类型切换按钮
                $container.find('#dnd-creator-type-pc').on('click', function() {
                    if (state.currentStep !== 'init') return;
                    state.characterType = 'pc';
                    UIRenderer.saveCreatorState();
                    UIRenderer.renderPanel('create');
                });
                $container.find('#dnd-creator-type-party').on('click', function() {
                    if (state.currentStep !== 'init') return;
                ...
                // 属性生成器按钮
                $container.find('#dnd-creator-stats-btn').on('click', function(e) {
                    if ($(this).prop('disabled')) return;
                    UIRenderer.showStatsGenerator(e);
                });

                // 开始创建按钮
                $container.find('#dnd-creator-start-btn').on('click', async function() {
                    ...
                    if (!state.apiConfig.url || !state.apiConfig.model) {
                        alert('请先在设置中配置 API 地址和模型');
                        UIRenderer.renderPanel('settings');
                        return;
                    }
                    ...
                    state.characterData = {};
                    UIRenderer.saveCreatorState(); // 保存状态变更
                    
                    // 发送初始系统消息给 AI
                    await UIRenderer.sendCreatorMessage(null, true);
                });
                
                // 重置按钮
                $container.find('#dnd-creator-reset-btn').on('click', function() {
                    if (confirm('确定要重新开始吗？当前对话和角色数据将被清除。')) {
                        // 重置为初始状态，但保留配置
                        UIRenderer._charCreatorState = {
                        ...
                        UIRenderer.saveCreatorState(); // 保存（覆盖）旧状态
                        UIRenderer.renderPanel('create');
                    }
                });
                
                // 发送按钮
                $container.find('#dnd-creator-send-btn').on('click', function() {
                    const $input = $container.find('#dnd-creator-user-input');
                    const text = $input.val().trim();
                    if (text) {
                        UIRenderer.sendCreatorMessage(text);
                        $input.val('');
                    }
                });
                
                // 回车发送
                $container.find('#dnd-creator-user-input').on('keypress', function(e) {
                ...
                // 确认创建
                $container.find('#dnd-creator-confirm-btn').on('click', async function() {
                    await UIRenderer.finalizeCharacterCreation();
                });
                
                // 继续修改
                $container.find('#dnd-creator-modify-btn').on('click', function() {
                    state.currentStep = 'chatting';
                    UIRenderer.saveCreatorState();
                    UIRenderer.renderPanel('create');
                });
            ...
            handleCreatorOption: (value, isMulti) => {
                // 暂时只支持单选，多选后续扩展
                // 模拟用户输入
                UIRenderer.sendCreatorMessage(value);
            },

            // 发送消息给 AI
            sendCreatorMessage: async (userMessage, isInitial = false) => {
                const { $ } = getCore();
                const state = UIRenderer._charCreatorState;
                const $chatHistory = $('#dnd-creator-chat-history');
                const $input = $('#dnd-creator-user-input');
                    ...
                    if (userMessage) {
                        ...
                        const userMsgHtml = `
                            <div class="dnd-chat-msg dnd-anim-entry" style="background:rgba(52, 152, 219, 0.1); border-left:3px solid #3498db; padding:10px 12px; margin-bottom:8px; border-radius:4px;">
                                ...
                                <div style="color:var(--dnd-text-main);line-height:1.5;white-space:pre-wrap;">${UIRenderer.formatChatMessage(userMessage)}</div>
                            </div>`;
                        $chatHistory.append(userMsgHtml);
                        $chatHistory.scrollTop($chatHistory[0].scrollHeight);
                        
                        state.conversationHistory.push({ role: 'user', content: userMessage });
                        UIRenderer.saveCreatorState(); // 保存
                    } else if (isInitial) {
    ...
    现在开始与用户对话。`;
                    ...
                    if (response) {
                        ...
                        const aiMsgHtml = `
                            <div class="dnd-chat-msg dnd-anim-entry" style="background:rgba(155, 89, 182, 0.1); border-left:3px solid #9b59b6; padding:10px 12px; margin-bottom:8px; border-radius:4px;">
                                ...
                                <div style="color:var(--dnd-text-main);line-height:1.5;white-space:pre-wrap;">${UIRenderer.formatChatMessage(response)}</div>
                            </div>`;
                        ...
                        if (optMatch) {
                            try {
                                ...
                                UIRenderer.renderChatOptions(optData);
                        ...
                        if (dataMatch) {
                            try {
                                ...
                                state.currentStep = 'reviewing';
                                // 更新预览区域
                                $('#dnd-creator-preview').html(UIRenderer.renderCharacterPreview(state.characterData));
            ...
            // 完成角色创建，保存数据
            finalizeCharacterCreation: async () => {
                const { $ } = getCore();
                const state = UIRenderer._charCreatorState;
                ...
                try {
                    ...
                    if (spells.length > 0 && skillLibTable && skillLinkTable) {
                        spells.forEach(spell => {
                            ...
                            // 先删除旧的关联 (如果有)
                            // 暂不删除，直接追加，依靠ID去重或允许重复? 最好是先清理该角色的该技能
                            // 这里简化：直接添加
                    ...
                    UIRenderer.saveCreatorState(); // 保存状态
                    UIRenderer.renderPanel('create');
                    
                    // 显示成功通知
                    alert(`🎉 ${isPC ? '主角' : '队友'} "${data.name}" 创建成功！`);
            ...
            showModal: (title, contentHtml) => {
                ...
                const $content = $('#dnd-modal-content');
                if ($overlay.length && $content.length) {
                    $content.html(`<h3>${title}</h3><div>${contentHtml}</div>`);
            ...
            showCharacterCard: (char, clickEvent) => {
                ...
                if (clickEvent && clickEvent.clientX !== undefined) {
                    UIRenderer._lastClickPos = { x: clickEvent.clientX, y: clickEvent.clientY };
                }
                ...
                setTimeout(() => {
                    ...
                    // [修复] 添加 party-bar-item 到排除列表
                    // [修复] 添加 dnd-logo-container 到排除列表 (防止点击 logo 关闭面板)
                ...
                // 构建 HTML
                const detailAvatarHtml = UIRenderer.renderAvatar(char['姓名'], charId, 48);
                    ...
                    <div class="dnd-detail-body">
                        ...
                        <div class="dnd-detail-section">
                            ...
                            <div style="font-size:12px;color:var(--dnd-text-main)">
                                ${res['法术位'] ? `<div style="margin-bottom:4px;">${UIRenderer.formatSpellSlots(res['法术位'])}</div>` : ''}
                                ${res['职业资源'] ? `<div>${UIRenderer.formatClassRes(res['职业资源'])}</div>` : ''}
                ...
                // 技能和专长点击事件已改用全局函数 (onclick) 处理，以确保事件触发
                ...
                if (isMobile) {
                    // 移动端：固定在顶部居中（CSS 已处理定位，只需添加 visible 类）
                    $card.css({
                        top: '10px',
                        left: '50%',
                        right: 'auto',
                    ...
                    // 桌面端：需要先测量尺寸再定位
                    // 临时显示以测量（使用 dnd-measuring 类避免触发动画）
                    ...
                    requestAnimationFrame(() => {
                        ...
                        // 获取点击位置
                        const clickX = UIRenderer._lastClickPos.x || winW / 2;
                        const clickY = UIRenderer._lastClickPos.y || 100;
                        ...
                        // 垂直方向：尽量与点击位置对齐，但确保完全可见
                        top = clickY - 50; // 稍微向上偏移
                        ...
                        // 应用位置并通过 visible 类触发 animation 入场动画
                        $card.css({
                            top: top + 'px',
                            left: left + 'px',
                            right: 'auto',
                            bottom: 'auto',
                            display: '', // 清除临时 display
                            visibility: '' // 清除临时 visibility
                        }).addClass('visible');
                    });
                }
            },
            ...
            showCombatUnitDetail: (unit, event) => {
                ...
                UIRenderer.showItemDetailPopup(html, event.clientX, event.clientY);
            },
            
            // 全局调用入口 (用于解决 onclick 绑定问题)
            handleSkillClick: (idx, event) => {
                console.log('[DND Dashboard] handleSkillClick', idx);
                if (event) { event.stopPropagation(); event.preventDefault(); }
                
                // 尝试从 DOM 获取数据
                // 注意：因为这里是全局调用，我们需要找到触发的元素
                // 但 event.target 可能只是 span，所以需要用 event.currentTarget 或者通过 ID
                // 这里为了简单，我们重新获取当前已打开卡片的角色数据
            ...
            renderHUD: () => {
                ...
                // 仅在 mini 状态下渲染
                if (UIRenderer.state !== 'mini') return;
                ...
                if (isCombat) {
                    UIRenderer.renderCombatHUD($body);
                } else {
                    UIRenderer.renderExploreHUD($body);
                }

                // [优化] 渲染常驻横向队伍栏 (替代原有的折叠列表)
                UIRenderer.renderPartyBar($body);

                // [新增] 渲染主角法术位 (迷你版)
                UIRenderer.renderMiniSpellSlots($body);

                // [优化] 渲染快捷物品栏 (替代原有的下拉列表)
                UIRenderer.renderQuickInventory($body);

                // 渲染常驻资源栏
                UIRenderer.renderFooter($body);

                // [新增] 渲染行动队列面板 (如果有待执行行动)
                if (UIRenderer._actionQueue.length > 0) {
                    const $queuePanel = $(`
                        <div style="margin-top:10px;background:rgba(0,0,0,0.3);border:1px solid var(--dnd-border-gold);border-radius:4px;overflow:hidden;">
                            <div style="background:rgba(197, 160, 89, 0.1);padding:5px 10px;font-weight:bold;color:var(--dnd-text-highlight);display:flex;justify-content:space-between;align-items:center;">
                                <span>⏳ 待执行行动 (${UIRenderer._actionQueue.length})</span>
                                <div style="display:flex;gap:5px;">
                            ...
                            <div style="padding:5px 10px;font-size:12px;color:#ccc;">
                                ${UIRenderer._actionQueue.map((a, i) => `<div style="margin-bottom:2px;">${i+1}. ${a.desc}</div>`).join('')}
                            </div>
                        </div>
                    `);
            ...
            renderPartyBar: ($container) => {
                ...
                party.forEach((char, idx) => {
                    ...
                    const charId = char['PC_ID'] || char['CHAR_ID'] || char['姓名'];
                    const initial = UIRenderer.getNameInitial(char['姓名']);
                    const avatarUid = `party-avatar-${charId}-${idx}`;
                    
                    // 触发异步头像加载
                    setTimeout(() => UIRenderer.loadAvatarAsync(charId, avatarUid), 0);
                    
                    // [新增] 检查是否为当前操控角色
                    const isControlled = (UIRenderer._controlledCharId === charId);
                ...
                $el.find('.party-bar-item').on('click', function(e) {
                    ...
                    if (char) UIRenderer.showCharacterCard(char, e);
                });
            ...
            renderQuickInventory: ($container) => {
                ...
                $el.find('.dnd-quick-item').on('click', function(e) {
                    ...
                    const item = items.find(i => (i['物品ID'] === itemId) || (i['物品名称'] === itemId));
                    if (item) UIRenderer.showMiniItemActions(item, e);
                });
            ...
            showMiniItemActions: (itemOrId, e) => {
                ...
                UIRenderer.showItemDetailPopup(html, x, y);
            },

            handleItemAction: (itemId, action, currentQty) => {
                UIRenderer.hideDetailPopup();
                ...
                if (action === 'equip') {
                    ...
                    if (confirm(`确定要${actionName} 1 个 ${itemId} 吗？`)) {
            ...
            renderCombatHUD: ($container) => {
                ...
                // 左侧：迷你地图
                const turnRes = UIRenderer._turnResources;
                html += `
                    <div style="display:flex;flex-direction:column;gap:5px;">
                        <div class="dnd-hud-minimap" id="dnd-hud-minimap-content"></div>
                        ...
                        <div style="display:flex;gap:5px;">
                            ...
                            <button class="dnd-clickable" style="
                                ...
                                font-weight: bold;
                                font-size: 12px;
                            " onclick="window.DND_Dashboard_UI.showCombatSkillList(event)">✨ 技能</button>
                        </div>
                    </div>
                `;
                
                // 右侧：战斗列表 (改为从 COMBAT_Encounter 读取)
                ...
                if (encounters && encounters.length > 0) {
                    ...
                    sorted.forEach(unit => {
                        ...
                        // 尝试获取头像 (如果是 PC 或有记录的 NPC)
                        const charIdCombat = unit['单位名称']; // 简单起见使用名称作为ID
                        const storedAvatarCombat = UIRenderer.avatarStorage.get(charIdCombat);
                        const initialCombat = UIRenderer.getNameInitial(unit['单位名称']);
                        ...
                        html += `
                            <div class="dnd-mini-char dnd-hud-entry ${isActive ? 'active' : ''}" style="${isEnemy ? 'border-left-color:var(--dnd-accent-red) !important;' : ''}">
                                <div class="dnd-mini-char-avatar dnd-avatar-container ${isActive ? 'dnd-active-turn' : ''}" data-char-id="${charIdCombat}" style="overflow:hidden;background:linear-gradient(135deg, #2a2a2e 0%, #1a1a1c 100%);position:relative;cursor:pointer;border-color:${isEnemy?'var(--dnd-accent-red)':'var(--dnd-border-gold)'};" title="${unit['单位名称']}">
                                    ${storedAvatarCombat 
                                        ? `<img src="${storedAvatarCombat}" style="width:100%;height:100%;object-fit:cover;position:absolute;top:0;left:0;" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"><div class="dnd-avatar-initial" style="display:none;width:100%;height:100%;align-items:center;justify-content:center;color:${isEnemy?'#ff6b6b':'var(--dnd-text-highlight)'};font-weight:bold;font-size:16px;">${initialCombat}</div>`
                                        : `<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;color:${isEnemy?'#ff6b6b':'var(--dnd-text-highlight)'};font-weight:bold;font-size:16px;">${initialCombat}</div>`
                                    }
                                </div>
                ...
                $container.find('.dnd-mini-char-avatar').on('click', function(e) {
                    ...
                    if (unit) {
                        UIRenderer.showCombatUnitDetail(unit, e);
                    }
                });

                // 渲染资源消耗
                if (UIRenderer.renderResourceConsumption) {
                    UIRenderer.renderResourceConsumption($('#dnd-combat-resource-panel'));
                }

                // 渲染迷你地图点
                UIRenderer.renderMiniMap($('#dnd-hud-minimap-content'));
            },

            renderMiniMap: async ($el) => {
                const { $ } = getCore();
                const global = DataManager.getTable('SYS_GlobalState');
                ...
                if (!isCombat) {
                    ...
                    if ($innerMap.length === 0) {
                        ...
                        // 绑定缩放
                        UIRenderer.bindMapZoom($el, $innerMap);
                    }
                            ...
                            setTimeout(() => {
                                try {
                                    ...
                                    if (viewBox) {
                                        ...
                                        // 应用缩放
                                        if (coverScale > 1.0) {
                                            UIRenderer._mapZoom.scale = coverScale;
                                            // Reset pan
                                            UIRenderer._mapZoom.panX = 0;
                                            UIRenderer._mapZoom.panY = 0;
                ...
                if ($innerMap.length === 0 ||
                    ...
                    $innerMap.on('click', (e) => {
                        ...
                        const grid = UIRenderer.getGridFromEvent(e, $el, $innerMap);
                        UIRenderer.handleMapInteraction(grid.x, grid.y);
                    });
                ...
                currentTokens.forEach(item => {
                    ...
                    if ($token.length > 0) {
                        ...
                        const initialToken = UIRenderer.getNameInitial(item['单位名称']);
                        ...
                        // [新增] 异步加载头像
                        setTimeout(() => UIRenderer.loadAvatarAsync(avatarKey, uid), 0);
                        
                        // 绑定点击事件
                        $token.on('click', (e) => {
                            e.stopPropagation();
                            if (UIRenderer._targetingMode.active) {
                                const p = DataManager.parseValue(item['坐标'], 'coord');
                                if (p) UIRenderer.handleMapInteraction(p.x||1, p.y||1);
                            } else {
                                if (enc) UIRenderer.showCombatUnitDetail(enc, e);
                            }
                        });
                ...
                const activeChar = UIRenderer.getControlledCharacter();
                ...
                if (UIRenderer._virtualPos) {
                    sourcePos = { ...UIRenderer._virtualPos };
                    sourceFound = true;
                    // 渲染虚拟位置 (Ghost)
                ...
                if (UIRenderer._targetingMode.active && sourceFound) {
                    const range = UIRenderer._targetingMode.range || 1;
            ...
            regenerateMap: async (locationName, mode) => {
                const { $ } = getCore();
                if (!confirm('确定要重新绘制地图图片吗？结构将保持不变。')) return;
                ...
                try {
                    ...
                    UIRenderer.renderHUD(); // Restore UI
                }
            },

            // [新增] 处理地图交互
            handleMapInteraction: (gridX, gridY) => {
                const state = UIRenderer._targetingMode;
                const encounters = DataManager.getTable('COMBAT_Encounter');
                const mapData = DataManager.getTable('COMBAT_BattleMap');
                const activeChar = UIRenderer.getControlledCharacter();
                ...
                if (state.active) {
                    ...
                    // 2. 检查距离
                    if (dist > state.range) {
                        alert(`目标超出范围！(距离: ${dist * 5}尺 / 射程: ${state.range * 5}尺)`);
                        return;
                    }
                ...
                `;
                // 计算屏幕坐标显示菜单
                // 由于 handleMapInteraction 无法直接获取 event 坐标，我们使用 grid 坐标反算中心点
                // 但这比较复杂，不如简单显示在屏幕中央或让用户再次确认
                // 为了简化，直接在 HUD 中间显示，或者尝试获取最后一次点击位置
                // 这里我们使用 showItemDetailPopup 并传入固定位置或 HUD 位置
                const $hud = $('#dnd-mini-hud');
                const hudRect = $hud[0].getBoundingClientRect();
                UIRenderer.showItemDetailPopup(menuHtml, hudRect.left + hudRect.width/2, hudRect.top + hudRect.height/2);
            },
            ...
            executeAction: (type, data, costType) => {
                ...
                // 特殊技能自动化逻辑
                const skillName = (UIRenderer._targetingMode.skillName || '').toLowerCase();
                let extraDesc = '';
                ...
                if (type === 'move') {
                    ...
                    if (UIRenderer._turnResources.movement < cost) {
                        alert(`移动距离不足！剩余: ${UIRenderer._turnResources.movement}尺, 需要: ${cost}尺`);
                        return; // 阻止执行
                    }
                    UIRenderer._turnResources.movement -= cost;
                    desc = `移动到了 ${coord} (消耗 ${cost}尺)`;
                    // 更新虚拟位置
                    UIRenderer._virtualPos = { x, y };
                } else {
                    // 动作/附赠/反应消耗
                    ...
                    if (UIRenderer._turnResources[key] !== undefined) {
                        if (UIRenderer._turnResources[key] <= 0) {
                            alert(`没有足够的 ${costType}！`);
                            return; // 阻止执行
                        }
                        UIRenderer._turnResources[key]--;
                    }
                    
                    const skill = UIRenderer._targetingMode.skillName;
                    if (target) {
                ...
                // 加入队列
                UIRenderer._actionQueue.push({ type, data, desc, charName });
            ...
            // [新增] 提交行动队列
            commitActions: () => {
                if (UIRenderer._actionQueue.length === 0) return;
                
                // 获取行动的主体名称
                const first = UIRenderer._actionQueue[0];
                const charName = first.charName || '我';
                
                // 合并文本
                const steps = UIRenderer._actionQueue.map(a => a.desc);
                let actionStr = '';
                ...
                UIRenderer.fillChatInput(finalStr);
                UIRenderer.clearActions();
            },

            // [新增] 清空行动队列
            clearActions: () => {
                UIRenderer._actionQueue = [];
                UIRenderer._virtualPos = null;
                UIRenderer.resetActionEconomy(); // 重置动作经济
            },
            ...
            bindMapZoom: ($container, $innerMap) => {
                ...
                containerEl.addEventListener('wheel', (e) => {
                    ...
                    applyTransform();
                    // Debounce save
                    if (UIRenderer._zoomSaveTimer) clearTimeout(UIRenderer._zoomSaveTimer);
                    UIRenderer._zoomSaveTimer = setTimeout(() => safeSave(CONFIG.STORAGE_KEYS.MAP_ZOOM, state.scale), 500);
                }, { passive: false });
            ...
            renderExploreHUD: ($container) => {
                ...
                // 异步渲染地图
                UIRenderer.renderMiniMap($mapContainer);
                
                // 1. 渲染行动选项 (优先)
                UIRenderer.renderActionOptions($container);
                ...
                if (quests && quests.length > 0) {
                    ...
                    `;
                    const $el = $(qHtml);
                    $el.on('click', (e) => UIRenderer.showQuestTooltip(activeQ, e.clientX, e.clientY));
                    $container.append($el);
                }
            },
            ...
            renderMiniSpellSlots: ($container) => {
                ...
                if (res && res['法术位']) {
                    const slotsHtml = UIRenderer.formatSpellSlots(res['法术位'], true); // 启用 mini 模式
                    if (slotsHtml) {
                        const $el = $(`
            ...
            renderPartyCollapse: ($container) => {
                const { $ } = getCore();
                const party = DataManager.getPartyData();
                
                // 状态保存，防止重绘时折叠状态丢失
                const isExpanded = UIRenderer._partyExpanded || false;
                
                let html = `
                <div class="dnd-hud-party-collapse">
                    <div class="dnd-party-header" id="dnd-party-toggle">
                        <span>👥 冒险队伍 (${party.length})</span>
                        <span>${isExpanded ? '▲' : '▼'}</span>
                    </div>
                    <div class="dnd-party-list ${isExpanded ? 'expanded' : ''}">`;
                ...
                party.forEach((char, idx) => {
                    ...
                    const charIdCollapse = char['PC_ID'] || char['CHAR_ID'] || char['姓名'];
                    const storedAvatarCollapse = UIRenderer.avatarStorage.get(charIdCollapse);
                    const initialCollapse = UIRenderer.getNameInitial(char['姓名']);
                    html += `
                        <div class="dnd-party-detail-row">
                            <div class="party-list-avatar dnd-avatar-container" data-idx="${idx}" data-char-id="${charIdCollapse}" style="width:24px;height:24px;border-radius:50%;overflow:hidden;border:1px solid #555;flex-shrink:0;margin-right:8px;cursor:pointer;background:linear-gradient(135deg, #2a2a2e 0%, #1a1a1c 100%);display:flex;align-items:center;justify-content:center;">
                                ${storedAvatarCollapse 
                                    ? `<img src="${storedAvatarCollapse}" style="width:100%;height:100%;object-fit:cover;">` 
                                    : `<span style="color:var(--dnd-text-highlight);font-weight:bold;font-size:12px;">${initialCollapse}</span>`
                                }
                            </div>
                            <div style="font-weight:bold;min-width:60px;">${char['姓名']}</div>
                            <div style="flex:1">
                                ...
                                <div class="dnd-micro-bar hp"><div class="dnd-micro-bar-fill" style="width:${hpPercent}%"></div></div>
                            </div>
                            <div style="font-size:10px;color:#aaa;text-align:right;">
                                <div>被动: ${char['被动感知']||'10'}</div>
                            </div>
                        </div>
                    `;
                });
                ...
                // 绑定头像点击事件
                $el.find('.party-list-avatar').on('click', function(e) {
                    e.stopPropagation();
                    const idx = $(this).data('idx');
                    const char = party[idx];
                    if (char) UIRenderer.showCharacterCard(char, e);
                ...
                $el.find('#dnd-party-toggle').on('click', function() {
                    const list = $(this).next('.dnd-party-list');
                    const expanded = list.hasClass('expanded');
                    if (expanded) {
                        list.removeClass('expanded');
                        $(this).find('span:last-child').text('▼');
                        UIRenderer._partyExpanded = false;
                    } else {
                        list.addClass('expanded');
                        $(this).find('span:last-child').text('▲');
                        UIRenderer._partyExpanded = true;
                    }
                });
            },
            ...
            renderFooter: ($container) => {
                ...
                $footerEl.find('.dnd-footer-btn').on('click', function(e) {
                    ...
                    switch(action) {
                        case 'inventory': UIRenderer.showInventoryPanel(e); break;
                        case 'equipment': UIRenderer.showEquipmentPanel(e); break;
                        case 'faction': UIRenderer.showFactionPanel(e); break;
                        case 'spellbook': UIRenderer.showSpellBook(e); break;
                        case 'dice': UIRenderer.showQuickDice(e); break;
                        case 'manual-update': UIRenderer.triggerManualUpdate(e); break;
                    }
                });
            ...
            showInventoryPanel: (event) => {
                ...
                if (!items || items.length === 0) {
                    UIRenderer.showItemDetailPopup('<div style="text-align:center;color:#888;">🎒 背包空空如也</div>', event.clientX, event.clientY);
                    return;
                }
            ...
            showEquipmentPanel: (event) => {
                const items = DataManager.getTable('ITEM_Inventory');
                if (!items) {
                    UIRenderer.showItemDetailPopup('<div style="text-align:center;color:#888;">⚔️ 无装备数据</div>', event.clientX, event.clientY);
                    return;
                }
            ...
            showCombatSkillList: (event) => {
                ...
                // 获取当前操控的角色
                const current = UIRenderer.getControlledCharacter();
            ...
            filterSpells: () => {
                ...
                $('.dnd-spell-group-header').each(function() {
                    ...
                    // 注意：这里假设 spell item 是按组顺序排列的，或者通过 data-level 关联
                    // 更健壮的做法是选择所有 data-level=lvl 的 visible item
            ...
            exportPartyToFile: () => {
                ...
                if (!data) {
                    PresetSwitcher.showNotification('⚠️ 无队伍数据可导出');
                    return;
                }
                ...
                try {
                    ...
                    PresetSwitcher.showNotification(`✅ 已导出 ${data.party.length} 个角色`);
                    Logger.info('[DND Dashboard] 队伍数据已导出');
                } catch (err) {
                    console.error('[DND Dashboard] 导出失败:', err);
                    PresetSwitcher.showNotification('❌ 导出失败');
                }
            },
                ...
                input.onchange = async (e) => {
                    ...
                    try {
                        ...
                        if (!data.party || !Array.isArray(data.party)) {
                            PresetSwitcher.showNotification('⚠️ 无效的队伍数据格式');
                            return;
                        }
                        
                        // 显示确认对话框
                        const confirmMsg = `确定要导入 ${data.party.length} 个角色吗？\n\n角色列表:\n${data.party.map(c => '- ' + (c['姓名'] || c['CHAR_ID'])).join('\n')}`;
                        
                        if (!confirm(confirmMsg)) {
                            return;
                        }
                        
                        const result = await DataManager.importPartyData(data);
                        
                        if (result.success) {
                            PresetSwitcher.showNotification(`✅ ${result.message}`);
                            Logger.info('[DND Dashboard] 队伍数据已导入:', result);
                            ...
                            PresetSwitcher.showNotification(`⚠️ ${result.message}`);
                        }
                    } catch (err) {
                        console.error('[DND Dashboard] 导入失败:', err);
                        PresetSwitcher.showNotification('❌ 导入失败: ' + err.message);
                    }
                };
            ...
            importFVTTFromFile: () => {
                ...
                input.onchange = async (e) => {
                    ...
                    try {
                        ...
                        PresetSwitcher.showNotification('⏳ 正在解析 FVTT 数据...');
                        const result = await DataManager.importFVTTData(data);
                        
                        if (result.success) {
                            PresetSwitcher.showNotification(`✅ ${result.message}`);
                            // 刷新界面
                            setTimeout(() => UIRenderer.renderPanel('party'), 500);
                        } else {
                            alert(result.message);
                        }
                    } catch (err) {
                        console.error('FVTT Import Error:', err);
            ...
            // [新增] 手动更新数据 - 调用 manualUpdate API
            triggerManualUpdate: async (event) => {
                const { $ } = getCore();
                const $btn = $(event.currentTarget);
                const $icon = $btn.find('.dnd-refresh-icon');
                ...
                try {
                    ...
                    if (api && api.manualUpdate) {
                        Logger.info('[DND Dashboard] 正在执行手动更新...');
                        const result = await api.manualUpdate();
                        if (result) {
                            PresetSwitcher.showNotification('✅ 数据已刷新');
                            Logger.info('[DND Dashboard] 手动更新成功');
                        } else {
                            PresetSwitcher.showNotification('⚠️ 更新返回失败');
                            Logger.warn('[DND Dashboard] 手动更新返回 false');
                        }
                    } else {
                        Logger.warn('[DND Dashboard] manualUpdate API 不可用');
                        PresetSwitcher.showNotification('⚠️ 刷新API不可用');
                    }
                } catch (err) {
                    console.error('[DND Dashboard] 手动更新失败:', err);
                    PresetSwitcher.showNotification('❌ 刷新失败');
                } finally {
                    // 恢复按钮状态
                    ...
                    // 刷新 HUD 显示
                    setTimeout(() => UIRenderer.renderHUD(), 500);
                }
            },
            ...
            showQuickDice: (event) => {
                // 获取骰子池数据
                const poolData = UIRenderer.getDicePoolData();
            ...
            refreshDicePool: async () => {
                ...
                // 关闭并重新打开面板以刷新显示
                setTimeout(() => {
                    const pos = UIRenderer._lastClickPos || { x: coreWin.innerWidth / 2, y: 100 };
                    UIRenderer.hideDetailPopup();
                    UIRenderer.showQuickDice({ clientX: pos.x, clientY: pos.y });
                }, 500);
            },
            ...
            toggleQuickBar: (forceState = null) => {
                ...
                if (forceState !== null) {
                    UIRenderer.quickBarState = forceState;
                } else {
                    UIRenderer.quickBarState = !UIRenderer.quickBarState;
                }
                
                const $bar = $('#dnd-quick-bar');
                const $trigger = $('#dnd-quick-trigger');
                
                if (UIRenderer.quickBarState) {
            ...
            renderQuickBar: async (providedSlots = null) => {
                const { $ } = getCore();
                const $bar = $('#dnd-quick-bar');
                ...
                // 1. 立即渲染缓存（如果存在）或空状态
                render(UIRenderer._cachedQuickSlots || []);
                ...
                try {
                    ...
                    UIRenderer._cachedQuickSlots = slots; // Cache it
                    render(slots);
                } catch(e) {
            ...
            showQuickSlotSelector: () => {
                ...
                const winW = coreWin.innerWidth || $(coreWin).width();
                const winH = coreWin.innerHeight || $(coreWin).height();
                UIRenderer.showItemDetailPopup(html, winW/2 - 150, winH/2 - 200);
            },
            ...
            addQuickSlot: async (type, name, id, icon, level) => {
                name = name || '未命名';
                id = String(id || '');
                icon = icon || '❓';
                level = String(level || '');
                ...
                let slots = [];
                // 优先使用缓存
                if (UIRenderer._cachedQuickSlots) {
                    slots = [...UIRenderer._cachedQuickSlots];
                } else {
                    try {
                        const saved = await DBAdapter.getSetting(CONFIG.STORAGE_KEYS.QUICK_SLOTS);
                        if (saved) slots = typeof saved === 'string' ? JSON.parse(saved) : saved;
                ...
                // 1. 立即更新缓存和UI (乐观更新)
                UIRenderer._cachedQuickSlots = slots;
                UIRenderer.renderQuickBar(slots);
                UIRenderer.hideDetailPopup();
            ...
            removeQuickSlot: async (index) => {
                if (!confirm('确定要移除此快捷方式吗？')) return;
                
                let slots = UIRenderer._cachedQuickSlots || [];
                // 如果缓存为空，尝试读取
                if (slots.length === 0) {
                    try {
                        const saved = await DBAdapter.getSetting(CONFIG.STORAGE_KEYS.QUICK_SLOTS);
                ...
                DBAdapter.getSetting(CONFIG.STORAGE_KEYS.QUICK_SLOTS).then(saved => {
                    ...
                    if (slot.type === 'item') {
                        ...
                        if (!hasLevel) {
                            ...
                            UIRenderer.fillChatInput(`我${action}：${slot.data.name}`);
                        } else {
                            // 升环/等级选择
                            const { $, window: coreWin } = getCore();
                            const winW = coreWin.innerWidth || $(coreWin).width();
                            ...
                            UIRenderer.showItemDetailPopup(html, winW/2 - 100, winH/2 - 100);
                        }
                    }
                });
            }
        ...
        // 将 UIRenderer 暴露到全局，以便 onclick 调用
